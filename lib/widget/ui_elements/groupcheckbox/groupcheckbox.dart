    ...
    if (mounted) {
      setState(() {
        if (i == widget.labels.length - 1) {
          if (isChecked) {
            _selected.clear();
            widget.labels.forEach((labels) => _selected.add(labels));
          } else {
            _selected.clear();
          }
        } else if (!isChecked && isAlreadyContained) {
          _selected.remove(widget.labels.elementAt(i));
          if (_selected.contains('Select all')) {
            _selected.removeLast();
          }
        } else if (isChecked && !isAlreadyContained) {
          _selected.add(widget.labels.elementAt(i));
          if (widget.labels.length - 1 == _selected.length) {
            _selected.add(widget.labels.last);
          }
        }
